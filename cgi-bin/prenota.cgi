...
$template->param(path => '<a href="load.cgi?page=prenotazioni">Prenotazioni</a> >> Prenota');
my $file = '../data/prenotazioni.xml';
my $parser = XML::LibXML->new();
my $xml = $parser->parse_file($file);

my @loop_news = $utils->getNews;
...
if ($name eq '' or ($fase == 2 and $name eq '')){		# fallimento, campi vuoti primo ingresso
    ...
    $template->param(NR_CAMPI => $utils->getFields($disciplina));
}

if($test_fase == 2){

    if($utils->checkform($disciplina, $campo, $data, $ora)){ # fallimento, prenotazione già presente nell'xml
	$test = -1;
    }
...
#$template->param(DISCIPLINA=>$disciplina); # aggiorno il campo hidden disciplina, che fallbacka a Calcetto se lasciato undef
	...
	$test = 0;
	my $table = $utils->getWeek($disciplina, $campo, $data);
